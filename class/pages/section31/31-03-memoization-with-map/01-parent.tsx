...
export default function MemoizationWithMapParentPage(): JSX.Element {
  ...
  const onClickChangeData = (): void => {
    setData("영희는 오늘 저녁을 맛없게 먹었습니다.");
  };
  return (
    <>
      {/* {data.split(" ").map((el: string, index) => (
        <Word key={index} el={el} /> //1.memo시, key 또는 el이 변경된 부분만 리렌더링 됨(즉, "오늘", "먹었습니다"는 제외)
      ))} */}
      {data.split(" ").map((el: string, index) => (
        <Word key={uuidv4()} el={el} /> //1.memo를 해도, key 자체가 변경되어 props로 넘어가므로, 5개 모두 리렌더링됨.
      ))}
      <button onClick={onClickChangeData}>데이터 변경하기</button>
    </>
  );
}
